...
// Waits until the DOM is loaded, and then runs out JavaScript
$(document).ready(function() {
  ...
  $('#friend-list').on('click', 'li', function(e) {
    ...
    $.ajax({
      url: `http://rest.learncode.academy/api/1150/friends/${e.target.id}`,
      type: 'DELETE'
    }).done( () => {
      // When the ajax requests succeeds, remove this list-item from the HTML
      $(this).remove();
    }).fail( () => {
      // If our request fails, alert the user
